    ...
    <div class="flex min-h-screen">
        <div class="w-64 shadow-md p-6 text-white" style="background-color: #1d180f;">
            ...
            <nav class="space-y-2">
                ...
                <p class="text-sm px-4  " style="color:rgb(249, 180, 4);">{{$user->email}}</p>
                <p class="block  py-2 rounded text-white hover:bg-[#2a241a]">Country</p>
                <p class="text-sm px-4  " style="color:rgb(249, 180, 4);">{{$user->country->name}}</p>
                <p class="block  py-2 rounded text-white hover:bg-[#2a241a]">Type</p>
                <p class="text-sm px-4  " style="color:rgb(249, 180, 4);">{{$user->type}}</p>
            </nav>
            <div class="px-3 py-8">
                <a href="{{ route('users.editUserInfo',$user->id) }}" class="text-sm border border-gray-300 px-3 py-1 rounded inline-block">
                    Edit Profile
                </a>
            </div>
        </div>
